...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/apis/apisSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureApisCols';
...
const TableSampleApis = ({
  ...
  const {
    ...
    notify: apisNotify,
    refetch,
  } = useAppSelector((state) => state.apis);
  ...
  useEffect(() => {
    if (apisNotify.showNotification) {
      notify(apisNotify.typeNotification, apisNotify.textNotification);
    }
  }, [apisNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/apis/apis-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {apis && Array.isArray(apis) && !showGrid && (
        <ListApis
          apis={apis}
          loading={loading}
...
export default TableSampleApis;
