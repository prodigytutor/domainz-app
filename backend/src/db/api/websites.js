...
module.exports = class WebsitesDBApi {
  static async create(data, options) {
    ...
    await websites.setDomain(data.domain || null, {
      transaction,
    });

    await websites.setAsset_farm(data.asset_farm || null, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.domain = await websites.getDomain({
      transaction,
    });

    output.asset_farm = await websites.getAsset_farm({
      transaction,
    });
  ...
  static async findAll(filter, globalAccess, options) {
    ...
    if (filter) {
      ...
      if (filter.host) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('websites', 'host', filter.host),
        };
      }
    ...
    let { rows, count } = options?.countOnly
      ...
      : await db.websites.findAndCountAll({
          where: globalAccess ? {} : where,
          include,
          distinct: true,
          limit: limit ? Number(limit) : undefined,
  ...
  static async findAllAutocomplete(query, limit, globalAccess, organizationId) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('websites', 'url', query),
        ],
      };
    }
