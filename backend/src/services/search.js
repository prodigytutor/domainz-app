  ...
  static async search(searchQuery, currentUser, organizationId, globalAccess) {
    try {
      ...
      const tableColumns = {
        ...
        apis: ['url', 'host', 'api_credentials', 'monitoring_info'],
        ...
        hostings: ['provider', 'account_credentials'],

        websites: ['url', 'host', 'site_credentials', 'monitoring_info'],

        asset_farms: ['name'],

        subdomains: ['url', 'host', 'site_credentials', 'monitoring_info'],
      };
      const columnsInt = {
        domains: ['price'],
      };
